...
from attacks_latent import SchedulerExponential, mix_classes
from utils import load_model, grid_plot, class_sampler
...
parser.add_argument('--natural', dest='robust', action='store_false')
parser.set_defaults(robust=True)


# Optional
parser.add_argument('-name', dest="name", help = 'experiment name', default='grad_sample_madry')
...
epsilons_attack = SchedulerExponential(args.attack_initial, args.attack_gamma, args.attack_steps)
 ...
 # Load pretrained models from Madry
if args.robust:
    classifier = load_model(args.classifier, 'mnist.py', 'mnist_adv.pth', device=args.device)
    name = args.name + '_robust'
else:
    classifier = load_model(args.classifier, 'mnist.py', 'mnist_natural.pth', device=args.device)
    name = args.name + '_natural'
...
file_name = args.path_output + '/' + f'{name}_{args.goal_ratio}_{gen}_{args.seed}'.replace('.', '-') + '.pickle'
